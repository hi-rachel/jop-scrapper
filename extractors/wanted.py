    ...
    for list in jobs:
        ...
        for anchors in job_list:
            ...
            for a in anchors:
                ...
                company = a.find('div',
                                 class_="job-card-company-name").get_text(),
                location = a.find(
                    'div', class_="job-card-company-location").get_text()
                job_data = {
                    'link':
                    f"https://www.wanted.co.kr{link}",
                    'company':
                    " ".join(str(x) for x in company).replace(",", " "),
                    'location':
                    " ".join(str(x) for x in location).replace(",", " "),
                    'position':
                    " ".join(str(x) for x in title).replace(",", " "),
                }
